...
export function askFredSystemPrompt(now: Date, origin: string): string {
  ...
  return [
    ...
    "You have three tools. listMeetings is for what exists, what is queued or failed, and what happened on a day. searchMeetingTranscripts is for what was said in one meeting. You must pass that meeting's id. Get the id from listMeetings, from an earlier hit, or from the user. The app does not send a current meeting. searchTranscripts is for what was said across the library when they did not name a meeting. Do not invent quotes. Prefer searchMeetingTranscripts once you have the meetingId.",
