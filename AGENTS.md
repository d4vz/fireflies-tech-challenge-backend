...
- `src/lib/<capability>/index.ts` is the interface. The vendor adapter lives under it: `src/lib/blob/minio/minio-blob.ts`, `src/lib/transcribe/assemblyai/assemblyai-transcribe.ts`, `src/lib/transcribe/openai/openai-transcribe.ts`, `src/lib/summarize/openai/openai-summarize.ts`, `src/lib/embed/openai/openai-embed.ts`, `src/lib/video/ffmpeg/ffmpeg-video.ts`, `src/lib/queue/bullmq/bullmq-queue.ts`, `src/lib/auth/clerk/clerk-jwt.ts`. `src/lib/transcribe/create-transcribe.ts` picks the adapter from `config.yaml` `models.transcribe` (`assemblyai` or an OpenAI model name).
...
`src/create-app.ts`, `src/server.ts`, and `src/lib/transcribe/create-transcribe.ts` are the files that import a vendor folder under a capability (`src/lib/blob/minio`, `src/lib/transcribe/assemblyai`, `src/lib/transcribe/openai`, `src/lib/summarize/openai`, `src/lib/embed/openai`, `src/lib/video/ffmpeg`, `src/lib/queue/bullmq`, `src/lib/db/mongo`, `src/lib/auth/clerk`). `server.ts` calls `createTranscribe`. `create-app.ts` is the composition root: it authenticates after `/health` and receives Owned Meetings.
